...
to return-to-nest
    ...
    face one-of patches with [pcolor = orange]
    fd 1
...
to look-for-food
  let chemical-ahead? scent-at-angle 0
  let chemical-right? scent-at-angle 45
  let chemical-left? scent-at-angle -45

  ifelse (chemical-right? > chemical-ahead? or chemical-left? > chemical-ahead?)
  [ifelse chemical-right? > chemical-left?
    [rt 45]
    [lt 45]
  ]
    [if chemical-ahead? = 0
      [ifelse coin-flip? [right random max-turn-angle][left random max-turn-angle]  ; if coin-flip? is true,
...
to evaporate-chemical
  let x random-float 1
  ask patches with [chemical > 0]
